...
type GoogleBusinessStatus = {
  configured: boolean;
  connected: boolean;
  selectedLocation: GoogleBusinessLocation | null;
...
export default function WebsiteBeheerPage() {
  ...
  const [googleStatus, setGoogleStatus] = React.useState<GoogleBusinessStatus | null>(null);
  const [googleLoading, setGoogleLoading] = React.useState(true);
  const [googleSavingLocation, setGoogleSavingLocation] = React.useState(false);
  ...
  const loadGoogleStatus = React.useCallback(async () => {
    try {
      ...
      const data = await response.json();
      if (!response.ok) {
        throw new Error(data?.error || "Google Business Profile-status kon niet worden geladen.");
      }
      setGoogleStatus(data);
    } catch (err) {
      setGoogleError(err instanceof Error ? err.message : "Onbekende Google-fout.");
    } finally {
  ...
  React.useEffect(() => {
    ...
    if (googleConnected === "connected") {
      setMessage("Google Business Profile is gekoppeld.");
    }
    if (googleErrorParam) {
      setGoogleError(googleErrorParam);
    }
  }, [loadGoogleStatus]);

  async function selectGoogleLocation(value: string) {
    const selected = googleStatus?.locations.find(
      (location) => `${location.accountName}|${location.locationName}` === value
    );
    if (!selected) return;

    try {
      setGoogleSavingLocation(true);
      setGoogleError("");
      const response = await fetch("/api/google-business/location", {
        method: "POST",
      ...
      const data = await response.json();
      if (!response.ok) {
        throw new Error(data?.error || "Google-locatie opslaan is mislukt.");
      }
      await loadGoogleStatus();
    } catch (err) {
      setGoogleError(err instanceof Error ? err.message : "Onbekende Google-fout.");
    } finally {
      setGoogleSavingLocation(false);
    }
  }
        ...
        <section className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
          <div className="border-b border-slate-200 bg-slate-50 px-6 py-4">
            <h2 className="flex items-center gap-2 font-bold text-slate-950">
              <MapPin className="h-5 w-5 text-blue-600" /> Google Business Profile
            </h2>
            <p className="mt-1 text-sm text-slate-600">
              Stap 1: koppel het Google-bedrijfsprofiel. Er worden in deze fase nog geen openingstijden gewijzigd.
            </p>
          </div>
          ...
          <div className="space-y-4 p-6">
            ...
            {googleLoading ? (
              <div className="flex items-center gap-2 text-sm text-slate-600">
                <Loader2 className="h-4 w-4 animate-spin" /> Google-status laden…
              </div>
            ) : !googleStatus?.configured ? (
              <div className="rounded-xl border border-amber-200 bg-amber-50 p-4 text-sm text-amber-900">
                De Google-koppeling staat klaar, maar de Vercel-variabelen
                <strong> GOOGLE_BUSINESS_CLIENT_ID</strong> en
                <strong> GOOGLE_BUSINESS_CLIENT_SECRET</strong> ontbreken nog.
              </div>
            ) : !googleStatus.connected ? (
              <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                <div>
                  <div className="font-semibold text-slate-900">Nog niet gekoppeld</div>
                  <div className="mt-1 text-sm text-slate-600">
                    Meld één keer aan met het Google-account dat eigenaar of beheerder is van IJssalon Vincenzo.
                  </div>
                </div>
                <a
              ...
              <>
                <div className="flex items-center gap-2 rounded-xl border border-emerald-200 bg-emerald-50 p-4 text-sm font-semibold text-emerald-800">
                  <CheckCircle2 className="h-5 w-5" /> Google-account gekoppeld
                </div>

                {googleStatus.locations.length > 0 ? (
                  <label className="block max-w-xl">
                    <span className="mb-1.5 block text-sm font-semibold text-slate-700">
                      Bedrijfsprofiel
                    </span>
                    <select
                      value={
                        googleStatus.selectedLocation
                          ? `${googleStatus.selectedLocation.accountName}|${googleStatus.selectedLocation.locationName}`
                          : ""
                      }
                      onChange={(event) => selectGoogleLocation(event.target.value)}
                      disabled={googleSavingLocation}
                      className="h-11 w-full rounded-xl border border-slate-200 bg-white px-3 text-sm text-slate-950 outline-none transition focus:border-blue-400 focus:ring-4 focus:ring-blue-100"
                    >
                      <option value="">Kies het bedrijfsprofiel…</option>
                      {googleStatus.locations.map((location) => (
                        <option
                    ...
                    {googleSavingLocation && (
                      <span className="mt-2 block text-xs text-slate-500">Locatie opslaan…</span>
                    )}
                  </label>
                ) : (
                  <div className="rounded-xl border border-amber-200 bg-amber-50 p-4 text-sm text-amber-900">
                    Google is gekoppeld, maar er is nog geen Business Profile-locatie gevonden.
                  </div>
                )}

                {googleStatus.selectedLocation && (
                  <div className="text-sm text-slate-600">
                    Geselecteerd: <strong>{googleStatus.selectedLocation.title}</strong>.
                    De volgende stap wordt het gecontroleerd synchroniseren van de openingstijden.
                  </div>
                )}
              </>
            )}
