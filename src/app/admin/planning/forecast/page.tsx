...
function cleanHourForMonth(maand: number) {
  return maand === 3 ? 21 : 23; // maart: schoon 21:00, anders 23:00
}
...
export default function ForecastPlanningPage() {
  ...
  // Robuuste gemiddelden (winsorized uur → terug naar kwartieren)
  const [robust, setRobust] = useState<boolean>(false);
  const [winsorAlpha, setWinsorAlpha] = useState<number>(0.1); // 10%

  // Personeel toggle & parameters
  ...
  const [norm, setNorm] = useState<number>(100); // € omzet / med / kwartier
  const [costPerQ, setCostPerQ] = useState<number>(6.25); // €25/u all-in
  const [itemsPerQ, setItemsPerQ] = useState<number>(10); // 10 items / med / kwartier
  ...
  const [kCostPerQ, setKCostPerQ] = useState<number>(7.5); // €30/u
          ...
          <>
            ...
            {data?.staff_meta && (
              <div className="text-sm text-gray-700 flex flex-wrap gap-4">
                <span>
                  <b>avg €/item:</b> {fmtEUR2(data.staff_meta.avg_item_rev_month || 0)}
                </span>
                <span>
                  <b>cap €/med/kw:</b> {fmtEUR2(data.staff_meta.cap_rev_per_staff_q || 0)}
                </span>
                <span>
                  <b>maandbudget:</b> {fmtEUR0(data.staff_meta.monthBudget || 0)}
                </span>
                <span>
                  <b>occ:</b> {Math.round((data.staff_meta.occ_this || 0) * 100)}%
                </span>
              </div>
            )}

            {/* Instellingen personeel + keuken */}
            <div className="grid grid-cols-1 md:grid-cols-3 xl:grid-cols-8 gap-3">
              ...
              <div>
                ...
                <input
                  ...
                  step="0.01"
                  value={groei}
                  onChange={(e) => setGroei(Number(e.target.value))}
                  className="border rounded px-2 py-1 w-full"
                />
              </div>
              <div>
              ...
              <div>
                ...
                <input
                  type="number"
                  value={itemsPerQ}
                  onChange={(e) => setItemsPerQ(Number(e.target.value))}
                  className="border rounded px-2 py-1 w-full"
                />
              </div>
              ...
              <div>
                ...
                <input
                  ...
                  step={900}
                  value={kDayStart}
                  onChange={(e) => setKDayStart(e.target.value)}
                  className="border rounded px-2 py-1 w-full"
                />
              </div>
              <div>
                ...
                <input
                  ...
                  min={0}
                  value={kDayCount}
                  onChange={(e) => setKDayCount(Number(e.target.value))}
                  className="border rounded px-2 py-1 w-full"
                />
              </div>
              <div>
                ...
                <input
                  ...
                  min={0}
                  value={kEveCount}
                  onChange={(e) => setKEveCount(Number(e.target.value))}
                  className="border rounded px-2 py-1 w-full"
                />
              </div>
              <div className="md:col-span-1 xl:col-span-2">
                ...
                <input
                  ...
                  step="0.01"
                  value={kCostPerQ}
                  onChange={(e) => setKCostPerQ(Number(e.target.value))}
                  className="border rounded px-2 py-1 w-full"
                />
              </div>
            </div>
      ...
      {data?.ok && Array.isArray(data?.weekdays) && (
        <div className="space-y-10">
          {data.weekdays.map((wd: any) => {
            // Kies de waarde voor heatmap/bedrag
            ...
            if (showStaff) {
              ...
              const frontPlanCost = wd.slots.reduce(
                (sum: number, s: any) => sum + (Number(s.staff_plan || 0) * costPerQ),
                0
              );
              ...
              headerCostNode = (
                <span className="text-sm text-gray-700">
                  ...
                  <span className="opacity-80">
                    {" "}
                    — front {fmtEUR2(frontPlanCost)}, keuken {fmtEUR2(kitchenCostTotal)}
                  </span>
                </span>
              );
            }
            ...
            return (
              <div key={wd.isodow} className="border rounded-lg p-4 shadow">
                ...
                {/* Tabel: per uur één rij, vier kwartieren per uur */}
