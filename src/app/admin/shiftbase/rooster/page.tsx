...
function addDays(iso: string, offset: number) { const d = new Date(iso+"T12:00:00"); d.setDate(d.getDate()+offset); return ymdLocal(d); }
function startOfISOWeekMonday(d: Date) { const t = new Date(d.getFullYear(), d.getMonth(), d.getDate()); const day = (t.getDay()+6)%7; t.setDate(t.getDate()-day); return t; }
function firstName(full?: string) { if(!full) return "Onbekend"; const w = full.trim().split(/\s+/)[0]; return w.split("-")[0]||w; }
function hoursBetween(a: string, b: string) { const [sh,sm]=a.split(":").map(Number); const [eh,em]=b.split(":").map(Number); let m=eh*60+em-(sh*60+sm); if(m<0)m+=1440; return m/60; }
...
export default function RoosterPage() {
  ...
  const { data: dayRooster, error: dayError } = useSWR<ShiftItem[]>(
    view==="day" ? `/api/shiftbase/rooster?datum=${selectedDate}` : null, fetcher
  );
  ...
  const wagesUrl = view==="week" && weekRooster
    ? `/api/shiftbase/wages-by-age?min_date=${weekDates[0]}&max_date=${weekDates[6]}&user_ids=${encodeURIComponent(weekUserIds.join(","))}`
    : null;
  ...
  return (
    <div className="p-4">
      ...
      <div className="flex flex-wrap items-center mb-4 gap-2">
        ...
        <input type="date" min="2024-01-01" max="2026-12-31" value={selectedDate} onChange={e=>setSelectedDate(e.target.value)} className="border px-2 py-1 rounded"/>
        <button onClick={()=>changeDay(1)} className="px-2 py-1 bg-gray-200 rounded">→</button>
      ...
      {view==="day" ? (
        <h1 className="text-xl font-bold mb-2">
          Rooster voor {new Date(selectedDate+"T12:00:00").toLocaleDateString("nl-NL",{weekday:"long",day:"2-digit",month:"2-digit",year:"numeric"})}
        </h1>
      ) : (
        <div className="flex items-center justify-between mb-2">
          <h1 className="text-xl font-bold">
            Rooster week {new Date(weekDates[0]+"T12:00:00").toLocaleDateString("nl-NL",{day:"2-digit",month:"2-digit"})}
            {" – "}
      ...
      {view==="day" ? (
        <>
          ...
          {dayRooster==null ? <p className="p-4">Laden…</p> : (
            orderDay.length===0 ? <p>Geen shifts gevonden voor deze dag.</p> : (
                    ...
                    <ul className="pl-4 list-disc">
                      {groep.map(it=>(
                        <li key={it.id} className="mb-1">
                          <span className="font-semibold">{it.Roster.starttime.slice(0,5)}–{it.Roster.endtime.slice(0,5)}</span>{" "}
        ...
        <>
            ...
            <div className="mb-3 rounded border p-3 text-sm bg-gray-50">
              ...
              <div className="grid sm:grid-cols-2 gap-2">
                <div>
                  ...
                  <div>Zonder geboortedatum (uniek):{" "}
                ...
                <div>
                  <div>Regels in loon_leeftijd: <strong>{wagesByAge.meta.rules_total}</strong></div>
                  <div>Bereik: {wagesByAge.meta.min_date} t/m {wagesByAge.meta.max_date}</div>
          ...
          {!weekRooster ? <p className="p-4">Laden…</p> : (
            <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 2xl:grid-cols-7">
              {weekDates.map((d)=>{
                ...
                Object.values(perShift).forEach(arr=> arr.sort((a,b)=> a.Roster.starttime.localeCompare(b.Roster.starttime)));
                ...
                let dayCost = 0, planned = 0, withWage = 0;
                ...
                for (const items of Object.values(perShift)) {
                  for (const it of items) {
                    ...
                    if (wage > 0 && hours > 0) { withWage += 1; dayCost += hours * wage; }
                  }
                }
                ...
                const hasRulesToday = meta?.rules_date_coverage?.[d] ?? false;
                ...
                return (
                  <div key={d} className="border rounded-lg p-2 text-xs leading-tight">
                    ...
                    {/* Controles per dag bij 0/x of op verzoek */}
                      ...
                      <div className="mt-2 space-y-1">
                        ...
                        {usersNoRule.length>0 && (
                          <div className="text-[11px] px-2 py-1 rounded bg-amber-50 text-amber-800 border border-amber-200">
                            Geen match met <code>loon_leeftijd</code> op {d}: {usersNoRule.join(", ")}
                          </div>
                        )}
