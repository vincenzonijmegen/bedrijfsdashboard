// src/app/admin/medewerker/[email]/instructies/page.tsx

import { db } from "@/lib/db";
import { notFound } from "next/navigation";
import Link from "next/link";

export default async function AdminInstructiesPage({ params }: { params: { email: string } }) {
  ...
  const statusResult = await db.query(
    `SELECT instructie_id, gelezen_op FROM gelezen_instructies WHERE email = $1`,
    [email]
  );
    ...
    <main className="max-w-3xl mx-auto p-6">
      ...
      <ul className="space-y-2 mt-4">
        {instructiesResult.rows.map((i) => (
          ...
          >
            <div>
              <p className="font-semibold">{i.nummer ? `${i.nummer}. ` : ""}{i.titel}</p>
              {gelezenIDs.has(i.id) ? (
            ...
            <Link
              href={`/admin/medewerker/${params.email}/instructies/${i.slug}`}
