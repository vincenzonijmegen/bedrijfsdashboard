...
function EntiteitCard({ e, busy, onSave }: any) {
  ...
  const [loonkostenGroei, setLoonkostenGroei] = useState(e.loonkosten_groei_pct ?? "");
  return <div className="rounded-xl border p-4"><div className="font-semibold">{e.naam}</div><div className="mt-3 space-y-2"><label className="block text-xs text-slate-500">Minimum kasbuffer</label><input className="w-full rounded-lg border px-3 py-2" type="number" step="0.01" value={buffer} onChange={x=>setBuffer(x.target.value)}/><label className="block text-xs text-slate-500">Omzetgroei % p/j</label><input className="w-full rounded-lg border px-3 py-2" type="number" step="0.1" value={groei} onChange={x=>setGroei(x.target.value)}/><label className="block text-xs text-slate-500">Loonkostengroei % p/j</label><input className="w-full rounded-lg border px-3 py-2" type="number" step="0.1" value={loonkostenGroei} onChange={x=>setLoonkostenGroei(x.target.value)}/><button disabled={busy} onClick={()=>onSave("PATCH",{type:"instelling",entiteit_id:e.id,minimum_kasbuffer:buffer,prognosegroei_pct:groei,loonkosten_groei_pct:loonkostenGroei})} className="mt-2 flex items-center gap-2 rounded-lg bg-slate-900 px-3 py-2 text-sm text-white"><Save className="h-4 w-4"/>Opslaan</button></div></div>;
}
