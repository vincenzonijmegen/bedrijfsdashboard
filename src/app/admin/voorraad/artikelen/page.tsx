...
interface InventarisatieProduct {
  id: number;
  naam: string;
  bestelnummer?: string;
...
export default function Productbeheer() {
  ...
  async function exporteerVoorraadopname() {
    try {
      const response = await fetch("/api/producten?inventarisatie=1");
      if (!response.ok) throw new Error("Ophalen van artikelen mislukt");

      const artikelen: InventarisatieProduct[] = await response.json();
      if (artikelen.length === 0) {
        alert("Er zijn geen actieve artikelen om te exporteren.");
        return;
      }
      ...
      const formuleRijen: number[] = [];
      const leverancierTotalen: { naam: string; start: number; einde: number; totaalRij: number }[] = [];

      let huidigeLeverancier = "";
      let leverancierStart = 0;
      let leverancierEinde = 0;

      for (const artikel of artikelen) {
        if (artikel.leverancier_naam !== huidigeLeverancier) {
          if (huidigeLeverancier) {
            const totaalRij = rows.length + 1;
            rows.push(["", "", "", "", `Totaal ${huidigeLeverancier}`, ""]);
            leverancierTotalen.push({ naam: huidigeLeverancier, start: leverancierStart, einde: leverancierEinde, totaalRij });
            rows.push([]);
          }

          huidigeLeverancier = artikel.leverancier_naam;
          rows.push([huidigeLeverancier]);
          rows.push(["Artikel", "Bestelnummer", "Besteleenheid", "Inkoopprijs", "Aantal voorraad", "Voorraadwaarde"]);
          leverancierStart = rows.length + 1;
        }

        const excelRij = rows.length + 1;
        rows.push([
          artikel.naam,
      ...
      rows.push([]);
      const eindTotaalRij = rows.length + 1;
      rows.push(["", "", "", "", "TOTALE VOORRAADWAARDE", ""]);
      ...
      leverancierTotalen.forEach(({ start, einde, totaalRij }) => {
        ws[`F${totaalRij}`] = { t: "n", f: `SUM(F${start}:F${einde})`, z: '€ #,##0.00' };
      });
      ws[`F${eindTotaalRij}`] = {
        t: "n",
        f: leverancierTotalen.map(({ totaalRij }) => `F${totaalRij}`).join("+"),
        z: '€ #,##0.00',
      };
      ...
      XLSX.utils.book_append_sheet(wb, ws, "Voorraadopname");
      XLSX.writeFile(wb, `Voorraadopname_${jaar}.xlsx`);
    } catch (error) {
      console.error(error);
      alert("Het Excel-bestand kon niet worden gemaakt.");
    }
  }
