...
export default function ContactenPage() {
  ...
  function updatePersoon(i: number, key: keyof Contactpersoon, value: string) {
    setCurrent(prev => {
      const personen = prev.personen ? [...prev.personen] : [];
      personen[i] = { ...personen[i], [key]: value };
      return { ...prev, personen };
    });
  }
  ...
  function deletePersoon(i: number) {
    setCurrent(prev => ({ ...prev, personen: prev.personen?.filter((_, idx) => idx !== i) }));
  }
              ...
              <div key={c.id} className="p-4 border rounded shadow">
                <div className="flex justify-between items-start">
                  ...
                  <div className="space-x-2">
                    <button onClick={() => openModal(c)} className="px-2 py-1 border rounded hover:bg-gray-100">Bewerk</button>
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white p-6 rounded-lg shadow-lg w-full max-w-2xl overflow-y-auto max-h-full">
            ...
            <form onSubmit={e => { e.preventDefault(); save(); }} className="space-y-4">
              ...
              <div className="flex flex-col">
                <label>Email</label>
                <input type="text" className="border rounded px-2 py-1" value={current.email || ''} onChange={e => updateField('email', e.target.value)} />
              </div>
              <div className="flex flex-col">
                <label>Website</label>
              ...
              <div className="flex justify-end space-x-2 pt-4">
                ...
                <button type="submit" className="px-4 py-2 bg-green-600 text-white rounded hover:bg-green-700">{current.id ? 'Opslaan' : 'Toevoegen'}</button>
              </div>
            </form>
          </div>
        </div>
