...
export async function DELETE(request: Request, { params }: { params: { verzuimId: string } }) {
  const verzuimId = params.verzuimId;
  await db.query('DELETE FROM ziekteverzuim WHERE id = $1', [verzuimId]);
  return NextResponse.json({ success: true });
}

export async function PATCH(request: Request, { params }: { params: { verzuimId: string } }) {
  const verzuimId = params.verzuimId;
