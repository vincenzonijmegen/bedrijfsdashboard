...
export async function GET(req: NextRequest) {
  try {
    const result = await db.query("SELECT * FROM skills ORDER BY categorie_id, naam");
...
export async function POST(req: NextRequest) {
  try {
    ...
    await db.query(
      "INSERT INTO skills (naam, categorie_id, actief) VALUES ($1, $2, true)",
      [naam, categorie_id]
    );
...
export async function PUT(req: NextRequest) {
  try {
    ...
    await db.query(
      "UPDATE skills SET naam = $1, categorie_id = $2, beschrijving = $3, actief = $4 WHERE id = $5",
