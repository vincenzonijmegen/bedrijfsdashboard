...
function isCronToegestaan(req: NextRequest) {
  const cronSecret = process.env.CRON_SECRET;

  if (!cronSecret) {
    console.error("❌ CRON_SECRET ontbreekt in omgeving");
    return false;
  }
...
export async function GET(req: NextRequest) {
  try {
    ...
    return NextResponse.json({
      success: true,
      verstuurd: aantalMails,
    });
  } catch (err) {
    console.error("❌ Fout bij skills-reminder cron:", err);

    return NextResponse.json(
      { error: "Skills-reminder cron mislukt." },
      { status: 500 }
    );
  }
}