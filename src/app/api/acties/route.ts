...
export async function POST(req: NextRequest) {
  try {
    const { lijst_id, tekst, deadline, verantwoordelijke, recurring } =
      await req.json();

    if (!lijst_id || !tekst) {
      return NextResponse.json(
        { error: "lijst_id en tekst zijn verplicht" },
        { status: 400 }
      );
    }
    ...
    const resultaat = await db.query(
      `
      INSERT INTO acties 
        (lijst_id, tekst, deadline, verantwoordelijke, volgorde, recurring)
      VALUES 
        ($1, $2, $3, $4, $5, $6)
      RETURNING *
      `,
  ...
  try {
    ...
    if (typeof recurring !== "undefined") {
      if (recurring !== "weekly" && recurring !== "none") {
        return NextResponse.json(
          { error: "recurring moet 'weekly' of 'none' zijn" },
          { status: 400 }
        );
      }
