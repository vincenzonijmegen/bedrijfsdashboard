...
// Haal de meest recente bestelling voor een leverancier
...
// Atomische update: verhoog of insert aantal voor een product
  ...
  try {
    ...
    const result = await pool.query(
      `INSERT INTO onderhanden_bestellingen (product_id, aantal, laatst_bewerkt)
       ...
       DO UPDATE SET
         aantal = onderhanden_bestellingen.aantal + 1,
         laatst_bewerkt = now()
...
// Verwijder alle items voor een leverancier of product
export async function DELETE(req: NextRequest) {
  ...
  if (leverancierId) {
    ...
    return NextResponse.json({ success: true, deleted: 'leverancier ' + leverancierId });
  }
