...
export default function StapVoorStapMetToets({ html, instructie_id, titel }: Props) {
  ...
  // Sla start- en eindtijd op in logs
  useEffect(() => {
  ...
  // Parse HTML in stappen en vragen
  useEffect(() => {
    const gebruiker = JSON.parse(localStorage.getItem("gebruiker") || "{}");
  ...
  }, [fase, feedback, heeftToets, vragen.length, stappen.length]);
  ...
  }, [fase, vragen.length, stappen.length]);
  ...
  const selectAntwoord = (letter: "A" | "B" | "C") => {
    ...
    if (juist) setAantalJuist((n) => n + 1);
      ...
      ...f,
      { vraag: vragen[index].vraag, gegeven: letter, gekozenTekst: vragen[index].opties[["A","B","C"].indexOf(letter)], }
    ]);
  ...
  // Render stap met ReactPlayer voor YouTube links
  const renderStep = (stepHtml: string) => {
    ...
    if (match) {
      ...
      return (
        <>
          ...
          <div
            className="border rounded p-4 bg-white shadow prose prose-blue max-w-none"
